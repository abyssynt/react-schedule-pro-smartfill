export const buildEmptyStaffStatsService = (mergedLeaveCodes = []) => ({
  work: 0,
  holidayLeave: 0,
  totalLeave: 0,
...
export const buildStaffStatsMapService = ({
  staffs = [],
  schedule = {},
  daysInMonth = [],
  mergedLeaveCodes = [],
  ...
  (staffs || []).forEach((staff) => {
    const stats = buildEmptyStaffStatsService(mergedLeaveCodes);
...
export const buildDailyStatsMapService = ({
  staffs = [],
  schedule = {},
  daysInMonth = [],
  getShiftGroupByCode = () => '',
...
export const buildRequiredCountMapService = ({
  daysInMonth = [],
  staffingConfig = {},
  getRequiredStaffingBucketByDay = () => 'weekday'
} = {}) => {
  const next = {};
  (daysInMonth || []).forEach((dayInfo) => {
...
export const getDemandHighlightStyleService = ({
  dateStr,
  rowKey,
  actualCount,
  requiredCountMap = {},
