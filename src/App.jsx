...
function ScheduleView({ changeScreen, colors, setColors, customHolidays, setCustomHolidays, specialWorkdays, setSpecialWorkdays, medicalCalendarAdjustments, setMedicalCalendarAdjustments, staffingConfig, setStaffingConfig, uiSettings, setUiSettings, customLeaveCodes, setCustomLeaveCodes, customColumns, setCustomColumns, customColumnValues, setCustomColumnValues, schedulingRulesText, setSchedulingRulesText, loadLatestOnEnter, onLatestLoaded, importedSchedulePayload, onImportedScheduleApplied, monthlySchedules, setMonthlySchedules, pendingOpenMonthKey, onPendingOpenHandled, year, setYear, month, setMonth, staffs, setStaffs, schedule, setSchedule }) {
  ...
  const getCycleDividerBorderStyle = (dateStr, width = 3) => (
    isCycleDividerDate(dateStr) ? `${width}px solid ${cycleDividerColor}` : undefined
  );
  const getCycleDividerHeaderShadow = (dateStr) => (
    isCycleDividerDate(dateStr) ? `inset -1px 0 0 ${cycleDividerPreviewColor}` : undefined
  );
...
(${d.weekStr})`), ...statHeaders];
    ...
    header.eachCell((cell, colNumber) => {
      ...
      const dayInfo = colNumber >= 2 && colNumber <= daysInMonth.length + 1 ? daysInMonth[colNumber - 2] : null;
      const dividerBorder = dayInfo && isCycleDividerDate(dayInfo.date)
        ? { style: 'medium', color: { argb: hexToExcelArgb(cycleDividerExcelColor, '#64748B') } }
        : { style: 'thin' };
      cell.border = {
        top: { style: 'thin' }, left: { style: 'thin' },
        bottom: { style: 'thin' }, right: dividerBorder
      };
            ...
            <thead>
              ...
              <tr>
                ...
                ${daysInMonth.map(d => {
                  ...
                  const dividerClass = isCycleDividerDate(d.date) ? ' cycle-divider' : '';
                ...
                {daysInMonth.map(d => (
                  <th
                    ...
                    style={{
                      ...
                      borderRight: getCycleDividerBorderStyle(d.date, 3) || undefined,
                      boxShadow: getCycleDividerHeaderShadow(d.date) || undefined
                    }}
                  >
