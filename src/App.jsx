...
import {
  DICT as SCHEDULE_DICT,
  SHIFT_GROUPS as DATA_SHIFT_GROUPS,
  GROUP_TO_DEMAND_KEY as DATA_GROUP_TO_DEMAND_KEY,
...
import { loadExcelJS, loadSheetJS } from './data/runtimeLoaderData';

import {
  normalizeImportedHalfWidth,
  getImportedRawNumberedLeaveValue,
  normalizeImportedShiftCode,
  isConfiguredImportedLeaveCode as importedIsConfiguredImportedLeaveCode,
  normalizeManualShiftCode as importedNormalizeManualShiftCode,
  getNormalizedManualCodeCandidates as importedGetNormalizedManualCodeCandidates
} from './data/importCodeData';

import { parseImportedExcelFiles as importedParseImportedExcelFiles } from './data/importMonthData';
...
import {
  blendHexColors,
  hexToExcelArgb,
  isFourWeekCycleEndDate as importedIsFourWeekCycleEndDate
} from './data/colorCycleData';

const DICT = SCHEDULE_DICT;
const SHIFT_GROUPS = DATA_SHIFT_GROUPS;
const GROUP_TO_DEMAND_KEY = DATA_GROUP_TO_DEMAND_KEY;
...
const normalizeManualShiftCode = (rawValue = '', allowedLeaveCodes = []) =>
  importedNormalizeManualShiftCode(rawValue, allowedLeaveCodes, getAllShiftCodes);

const getNormalizedManualCodeCandidates = (rawValue = '', allowedLeaveCodes = []) =>
  importedGetNormalizedManualCodeCandidates(rawValue, allowedLeaveCodes, getAllShiftCodes);
...
const isConfiguredImportedLeaveCode = (code = '', customLeaveCodes = []) =>
  importedIsConfiguredImportedLeaveCode(code, customLeaveCodes, getCodePrefix);

const parseImportedExcelFiles = (files = [], fallbackYear = new Date().getFullYear(), options = {}) =>
  importedParseImportedExcelFiles(files, fallbackYear, options, {
    loadSheetJS,
    getAllShiftCodes,
    getShiftGroupByCode,
    getCodePrefix
...
const isFourWeekCycleEndDate = (dateStr) => importedIsFourWeekCycleEndDate(dateStr, parseDateKey);
