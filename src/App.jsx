          ...
          <table className="w-max min-w-full border-collapse">
            ...
            <tbody>
              {groupedStaffs.map(({ group, staffs: groupStaffList }) => {
                const isWhiteGroup = group === '白班';
                const isCollapsed = Boolean(collapsedGroups[group]);

                if (isWhiteGroup && isCollapsed) {
                  return (
                    <React.Fragment key={group}>
                      <tr className="border-b border-slate-200 bg-slate-50/40">
                        <td
                          rowSpan={1}
                          className="sticky left-0 z-20 border-r text-center shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)]"
                          style={{ width: densityConfig.shiftWidth, minWidth: densityConfig.shiftWidth, backgroundColor: shiftColumnBgColor }}
                        >
                          <div className="relative flex items-center justify-center h-full" style={{ minHeight: densityConfig.rowMinHeight }}>
                            <button
                              type="button"
                              onClick={() => toggleGroupCollapse(group)}
                              className="absolute left-1 top-1 flex h-5 w-5 items-center justify-center rounded-md border border-slate-300 bg-white text-slate-700 hover:bg-slate-100"
                              aria-label="展開白班"
                              title="展開白班"
                            >
                              <Plus size={12} />
                        ...
                        <td
                          className={`sticky z-30 border-r shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)] ${densityConfig.nameCellPaddingClass}`}
                          style={{ left: densityConfig.shiftWidth, width: densityConfig.nameWidth, minWidth: densityConfig.nameWidth, backgroundColor: nameDateColumnBgColor }}
                        />
                        {daysInMonth.map(d => (
                          <td
                            key={d.date}
