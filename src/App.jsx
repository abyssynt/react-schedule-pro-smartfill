...
export default function App() {
  ...
  const [staffs, setStaffs] = useState([
    { id: 's1', name: '新成員' }, { id: 's2', name: '新成員' }, { id: 's3', name: '新成員' }, { id: 's4', name: '新成員' }, { id: 's5', name: '新成員' }, { id: 's6', name: '新成員' }, { id: 's7', name: '新成員' }
    , { id: 's8', name: '新成員' }, { id: 's9', name: '新成員' }, { id: 's10', name: '新成員' }, { id: 's11', name: '新成員' }, { id: 's12', name: '新成員' }, { id: 's13', name: '新成員' }, { id: 's14', name: '新成員' }
    , { id: 's15', name: '新成員' }
  ]);
  ...
  const exportToExcel = async () => {
    ...
    const headerRow = ['日期/姓名', ...daysInMonth.map(d => `${d.day}\n(${d.weekStr})`), '上班', '假日休', '總休', ...DICT.LEAVES];
  ...
  const exportToWord = () => {
    ...
    <body><h2 style="text-align:center;">${year}年${month}月 班表</h2><table><thead><tr><th>姓名</th>${daysInMonth.map(d => `<th class="${d.isHoliday ? 'holiday' : (d.isWeekend ? 'weekend' : '')}">${d.day}<br/>(${d.weekStr})</th>`).join('')}<th>上班</th><th>總休</th></tr></thead>
    <tbody>${staffs.map(staff => {
      const stats = getStaffStats(staff.id);
      return `<tr><td>${staff.name}</td>${daysInMonth.map(d => {
        const cellData = schedule[staff.id]?.[d.date];
  ...
  const loadHistory = (record) => {
    ...
    setStaffs(state.staffs); setSchedule(state.schedule);
  ...
  const addStaff = () => {
    ...
    setStaffs([...staffs, { id: newId, name: '新成員' }]);
  ...
  const moveStaff = (index, direction) => {
    ...
    const [movedStaff] = newStaffs.splice(index, 1);
    newStaffs.splice(targetIndex, 0, movedStaff);
    setStaffs(newStaffs);
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 font-sans overflow-x-hidden relative">
      ...
      <div className="max-w-[95vw] mx-auto mb-6 grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5 flex items-center justify-end gap-2">
          ...
          <button onClick={addStaff} className="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition-colors flex items-center gap-2 font-bold"><Plus size={20}/> 新增人員</button>
        </div>
      </div>
          ...
          <table className="w-max min-w-full border-collapse">
            <thead>
              <tr className="bg-slate-100 border-b-2 border-slate-200">
                <th className="sticky left-0 bg-slate-200 z-30 p-4 border-r font-black text-slate-700 w-36">日期/姓名</th>
                {daysInMonth.map(d => (
              ...
              {staffs.map((staff, index) => {
                ...
                return (
                  <tr key={staff.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    <td className="sticky left-0 bg-white z-10 border-r p-2 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)]">
                      <input type="text" value={staff.name} onChange={(e) => { const n = [...staffs]; n[index].name = e.target.value; setStaffs(n); }} className="w-full text-center py-2 font-bold text-slate-700 border-none rounded-lg focus:ring-2 focus:ring-blue-400 bg-transparent" />
                    </td>
                    {daysInMonth.map(d => {
                      const cellData = schedule[staff.id]?.[d.date];
                    ...
                    <td className="p-2"><div className="flex justify-center gap-1">
                      <button onClick={() => moveStaff(index, 'up')} disabled={index===0} className="text-slate-400 hover:text-blue-500 disabled:opacity-10"><ArrowUp size={14}/></button>
                      <button onClick={() => removeStaff(staff.id)} className="text-slate-400 hover:text-red-500"><Minus size={14}/></button>
                    </div></td>
                  </tr>
                );
              })}
            ...
            <tfoot className="bg-slate-100 border-t-2 border-slate-200">
              {['D', 'E', 'N', 'totalLeave'].map((rowKey) => (
                <tr key={rowKey}>
                  <td className="sticky left-0 bg-slate-200 z-10 border-r p-3 text-right text-xs font-bold text-slate-600">{rowKey === 'totalLeave' ? '當日休假' : `${rowKey} 班人數`}</td>
                  {daysInMonth.map(d => {
