          ...
          <table className="w-max min-w-full border-collapse">
            ...
            <tbody>
              {groupedStaffs.map(({ group, staffs: groupStaffList }) => {
                ...
                const visibleStaffs = isCollapsed ? [] : groupStaffList;
                const rowSpanCount = Math.max(1, visibleStaffs.length + 1);
                const summaryLabel = group === '白班' ? '白班上班' : group === '小夜' ? '小夜上班' : '大夜上班';
                ...
                <React.Fragment key={group}>
                  {visibleStaffs.map((staff, index) => {
                    ...
                    return (
                      <tr key={staff.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                        {index === 0 && (
                          <td rowSpan={rowSpanCount} className="sticky left-0 z-20 border-r text-center shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)]" style={{ width: densityConfig.shiftWidth, minWidth: densityConfig.shiftWidth, backgroundColor: shiftColumnBgColor }}>
                  ...
                  {visibleStaffs.length === 0 && (
                    <tr className="border-b border-slate-200 bg-slate-50/70">
                      <td rowSpan={1} className="sticky left-0 z-20 border-r text-center shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)]" style={{ width: densityConfig.shiftWidth, minWidth: densityConfig.shiftWidth, backgroundColor: shiftColumnBgColor }}>
                        <div className="flex items-center justify-center h-full" style={{ minHeight: densityConfig.rowMinHeight }}>
                          {showShiftLabels && (
                      ...
                      {daysInMonth.map(d => (
                        <td key={d.date} className="border-r" style={{ backgroundColor: d.isHoliday ? colors.holiday : (d.isWeekend ? colors.weekend : 'transparent'), opacity: d.isHoliday || d.isWeekend ? 0.9 : 1 }}></td>
                      ))}
                  ...
                  <tr className="border-b border-slate-200">
                    <td className="sticky left-0 z-20 border-r bg-slate-50 text-center align-middle shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)]" style={{ width: densityConfig.shiftWidth, minWidth: densityConfig.shiftWidth, backgroundColor: shiftColumnBgColor }}>
                      <button
                        type="button"
                        onClick={() => setCollapsedGroups(prev => ({ ...prev, [group]: !prev[group] }))}
                        className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-slate-300 bg-slate-100 text-slate-600 hover:bg-slate-200"
                        title={isCollapsed ? `展開${group}` : `收合${group}`}
                        aria-label={isCollapsed ? `展開${group}` : `收合${group}`}
                      >
                        <ChevronRight size={16} className={`transition-transform ${isCollapsed ? '-rotate-90' : 'rotate-90'}`} />
                      </button>
                    </td>
                    <td className={`sticky z-10 border-r text-center font-bold ${nameDateColumnFontSizeClass} ${densityConfig.footCellPaddingClass}`} style={{ left: densityConfig.shiftWidth, width: densityConfig.nameWidth, minWidth: densityConfig.nameWidth, backgroundColor: '#f8f1c9', color: nameDateColumnFontColor }}>
                      {summaryLabel}
                    </td>
                    {daysInMonth.map(d => (
                      <td key={d.date} className={`border-r text-center font-black ${tableFontSizeClass}`} style={{ backgroundColor: '#f8f1c9', color: tableFontColor }}>
                        {getDailyStats(d.date)[summaryKey] || ''}
                      </td>
                    ))}
                    <td colSpan={(showRightStats ? 3 : 0) + (showLeaveStats ? mergedLeaveCodes.length : 0) + (customColumns?.length || 0)} style={{ backgroundColor: '#f8f1c9' }}></td>
                  </tr>
                </React.Fragment>
              );
              })}
